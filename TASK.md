...
- [ ] 8-1. 難易度別定数定義（制限時間・速度係数）
  - cheat：60秒・1.0秒/文字
  - normal：90秒・0.65秒/文字
  - hard：120秒・0.4秒/文字
  - kichiku：150秒・0.25秒/文字
- [ ] 8-2. ワード別タイムアウト計算（ローマ字文字数 × 速度係数）
- [ ] 8-3. 総制限時間カウントダウン・ゲーム終了処理実装
- [ ] 8-4. タイムアウト時の自動ワード切り替え実装
- [ ] 8-5. ループ出題実装（ワードを使い切ったらシャッフルして継続）
- [ ] 8-6. スコア計算の再設計・実装（タイムアウト減点含む）
- [ ] 8-7. HUD 更新（総残り時間・ワード制限時間バーの表示）
- [ ] 8-8. リザルト画面の更新（新スコア表示対応）
- [ ] 8-9. `useTypingGame` フックのテスト更新（時間制対応）
- [ ] **Phase 8 完了チェック：build / dev / lint → コミット → プッシュ**
...
- **スコア計算**：タイムアウト時の減点量はPhase 8の設計時に決定
